...
\end{equation}
Inverting $\mathcal{M}'_{a}$ is considerably faster and less memory intensive compared to $\mathcal{M}_{a}$, even for large numbers of degrees of freedom; however, the correlations between the alignable structures is lost.
...
The former generally occurs once a sufficient amount of data is collected after a detector shutdown, in order to obtain a good baseline alignment for use in the remainder of the data collection period.
...
The latter occurs once data taking is complete.
A new set of alignment constants is derived for the full set of available data, and it is typically divided into several ``blocks'' to account for potential run-by-run misalignments.
