...
\end{equation}
a simple Lagrangian $\mathcal{L}$ can be written:
\begin{equation}
...
Isolating the potential term of the Lagrangian:
\begin{equation}
  V(\phi) = \mu^2\phi^{\dagger}\phi+\lambda(\phi^{\dagger}\phi)^2
  \label{eq:higgs_potential}
\end{equation}
a choice must be made on the sign of $\mu^2$, and the case of interest is for $\mu^2 < 0$. % \TODO{page 328, 334}
This results in the so-called ``Mexican hat potential'' shown in Figure~\ref{fig:mexican_hat}, which is minimized along the collection of points:
\begin{equation}
...
The VEV can be substituted back into the original Lagrangian in Equation~\ref{eq:complex_scalar_lagrangian}, and, following quite a bit of math~\cite{1984.halzen-martin}, a collection of mass terms can be identified:
\begin{equation}
